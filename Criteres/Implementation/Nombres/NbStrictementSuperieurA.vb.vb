Public Class NbStrictementSuperieurA(Of T As IElementFiltrable)
...
#Region "Prédicat"
    Public Overrides Function Accept(element As T) As Boolean
        If IsNothing(_GetElementValeur) Then Throw New NoNullAllowedException("Pour un critère de type strictement supérieur à, il faut spécifié la méthode GetValeur")
...
#Region "Arguments"
    ...
    Protected Overrides Function IsArgumentsInvalid(ParamArray args() As Object) As Boolean
        _NbArguments = 1
        _ArgumentsInvalidExceptionMessage = "pour le critère NbStrictementSuperieurA il faut définir un nombre non nulle comme argument"
